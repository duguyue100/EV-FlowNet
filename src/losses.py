...
def warp_images_with_flow(images, flow):
    ...
    as in Spatial Transformer Networks.
    """
    ...
    pos_x = tf.to_float(
        tf.expand_dims(tf.expand_dims(coord_x, axis=2), axis=0))+flow_x
    pos_y = tf.to_float(
        tf.expand_dims(tf.expand_dims(coord_y, axis=2), axis=0))+flow_y
...
def compute_smoothness_loss(flow):
    ...
    The neighborhood here is defined as the 8-connected region
    around each pixel.
    """
    flow_ucrop = flow[..., 1:, :]
    flow_dcrop = flow[..., :-1, :]
...
def compute_photometric_loss(prev_images, next_images,
                             event_images, flow_dict):
    """
    ...
    for i in range(len(flow_dict)):
        ...
        photometric_loss = charbonnier_loss(
            next_images_warped - prev_images_resize)
