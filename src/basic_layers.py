...
def general_conv2d(conv, name=None, channelsout=64,
                   ksize=3, strides=2, init_factor=0.1,
...
def upsample_conv2d(conv, name=None, channelsout=64, ksize=3, init_factor=0.1,
    ...
    """
    Upsample a tensor by a factor of 2 with fixed padding and then do normal
    conv2d on it. Similar operation to a transposed convolution,
    but avoids checkerboard artifacts.
    """
    if data_format == 'channels_first':
        conv = tf.transpose(conv, [0, 2, 3, 1])
        shape = tf.shape(conv)
        conv = tf.image.resize_images(
            conv, size=[shape[1]*2, shape[2]*2],
    ...
    conv = general_conv2d(
        conv, name=name, channelsout=channelsout, ksize=ksize, strides=1,
