...
class EVFlowNet():
    ...
    def _build_graph(self, global_step):
        ...
        with tf.name_scope('loss'):
            ...
            with tf.name_scope('weight_decay'):
                var = tf.get_collection(
                    tf.GraphKeys.TRAINABLE_VARIABLES, scope='vs/')
                for v in var:
                    wd_loss = tf.multiply(
        ...
        with tf.name_scope('optimizer'):
            ...
            learning_rate = tf.train.exponential_decay(
                self._args.initial_learning_rate,
                global_step,
    ...
    def train(self):
        ...
        # load data
        flow_dict, self._loss, self._optimizer, next_image_warped = \
            self._build_graph(global_step)
        ...
        with tf.device('/cpu:0'):
            event_img = tf.expand_dims(
                self._event_img_loader[:, :, :, 0] +
                self._event_img_loader[:, :, :, 1], axis=3)

            event_time_img = tf.reduce_max(
                self._event_img_loader[:, :, :, 2:4],
                axis=3,
                keepdims=True)
            ...
            color_wheel_rgb = draw_color_wheel_tf(
                self._args.image_width, self._args.image_height)

            # Appending letters to each title allows us to
