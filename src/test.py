...
def test(sess,
    ...
    event_image = tf.reduce_sum(
        event_image_loader[:, :, :, :2], axis=-1, keepdims=True)
    ...
    while not coord.should_stop():
        ...
        event_count_image = \
            (event_count_image*255/event_count_image.max()).astype(np.uint8)
        ...
        if args.gt_path:
            ...
            AEE, percent_AEE, n_points = flow_error_dense(
                gt_flow,
                pred_flow,
                event_count_image,
...
def main():
    args = configs()
    args.load_path = tf.train.latest_checkpoint(
        os.path.join(args.load_path,
        args.training_instance))

    sess = tf.Session()
    event_image_loader, prev_image_loader, next_image_loader, \
        timestamp_loader, n_ima = get_loader(
            args.data_path,
            1,
            args.image_width,
            args.image_height,
